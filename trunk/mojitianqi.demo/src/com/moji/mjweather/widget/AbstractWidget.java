...
public abstract class AbstractWidget extends AppWidgetProvider {
	...
	protected static void changeCityIndex(Context context) {
		if (Gl.getCurrentCityIndex() != 0
				|| WeatherData.getCityInfo(1 + Gl.getCurrentCityIndex()).mShowType != com.moji.mjweather.data.CityWeatherInfo.ShowType.ST_NOSET) {
			if (Gl.getCurrentCityIndex() >= 8
					|| WeatherData.getCityInfo(1 + Gl.getCurrentCityIndex()).mShowType == com.moji.mjweather.data.CityWeatherInfo.ShowType.ST_NOSET)
				Gl.saveCurrentCityIndex(0);
			else
				Gl.saveCurrentCityIndex(1 + Gl.getCurrentCityIndex());
			WeatherAlert.cancelAllAlertNotifications();
			WidgetManager.startWidgetService(context, null,
					WidgetManager.WidgetServiceType.UPDATE_NOW);
		}
	}
	...
	public void onDeleted(Context context, int ai[]) {
		MojiLog.v("AbstractWidget", (new StringBuilder()).append("onDeleted: ")
				.append(getWidgetType()).toString());
		super.onDeleted(context, ai);
	}

	public void onDisabled(Context context) {
		MojiLog.v("AbstractWidget", (new StringBuilder())
				.append("onDisabled: ").append(getWidgetType()).toString());
		try {
			WidgetManager.stopWidgetService(context, getWidgetType(),
					WidgetManager.WidgetServiceType.UPDATE);
	...
	public void onEnabled(Context context) {
		MojiLog.v("AbstractWidget", (new StringBuilder()).append("onEnabled: ")
				.append(getWidgetType()).toString());
	...
	public void onReceive(Context context, Intent intent) {
		MojiLog.v("AbstractWidget", (new StringBuilder()).append("onReceive: ")
				.append(getWidgetType()).toString());
		if (intent == null)
			return;
		...
		if (MojiLog.isDevelopMode())
			MojiLog.d(
					...
					(new StringBuilder()).append("context = ")
							.append(context.toString()).append(" getAction = ")
							.append(s).toString());
		if (!s.equals("android.intent.action.USER_PRESENT")) {
			...
			if (s.equals("android.intent.action.TIME_SET")
					|| s.equals("android.intent.action.DATE_CHANGED")
					|| s.equals("android.intent.action.TIMEZONE_CHANGED")) {
				Intent intent1 = new Intent("");
				intent1.setAction("com.moji.mjweather.widget.update.now");
				WidgetManager
						.startWidgetService(context, intent1, getWidgetType(),
								WidgetManager.WidgetServiceType.UPDATE);
			} else {
				onHotArea(context, s);
			}
	...
	public void onUpdate(Context context, AppWidgetManager appwidgetmanager,
			int ai[]) {
		MojiLog.v("AbstractWidget", (new StringBuilder()).append("onUpdate: ")
				.append(getWidgetType()).toString());
