...
public class WeatherMainActivity extends Activity implements
	...
	class WeathearTabCheckedCallBackImpl implements WeatherTabCheckedCallback {
	...
	class UpdateCallbackImpl implements WeatherSubscriber {
	...
	private void addUmengAd() {
				...
				public void onReportStart(Map map) {
					...
					do {
						...
						stringbuffer.append((new StringBuilder()).append(s)
								.append(": ").append(map.get(s).toString())
								.toString());
		...
		MojiLog.d("WeatherMainActivity", (new StringBuilder())
				.append("AdOn = ").append(Gl.getAdON()).toString());
		if (Gl.getAdON()) {
	...
	private void bindCheckNeedUpdate(Workspace workspace) {
		doUpdateCityWeather(true);
		workspace
				.setCheckNeedUpdateListener(new com.moji.mjweather.view.Workspace.OnCheckNeedUpdateListener() {
	...
	private void cancelUpdateWeather(CityWeatherInfo cityweatherinfo,
			boolean flag) {
	...
	private void doIntentCityManager() {
		if (WeatherData.getCityInfo(Gl.getCurrentCityIndex()).mShowType == com.moji.mjweather.data.CityWeatherInfo.ShowType.ST_UPDATING) {
			...
			Gl.setChangedCity(false);
			startActivityForResult(new Intent(this, CityManagerActivity.class),
					99);
		}
	}
	...
	private void doUpdateCityWeather(boolean flag) {
		CityWeatherInfo cityweatherinfo = WeatherData.getCityInfo(Gl
				.getCurrentCityIndex());
		if ((!flag || cityweatherinfo.mShowType == com.moji.mjweather.data.CityWeatherInfo.ShowType.ST_NEED_BE_UPDATE)
			...
			if (!Util.isConnectInternet(this)) {
				...
				mUpdateLayout = (LinearLayout) mWorkSpace.getChildAt(
						mWorkSpace.getCurrentScreen()).findViewById(0x7f0d0106);
				if (cityweatherinfo.mShowType == com.moji.mjweather.data.CityWeatherInfo.ShowType.ST_OK)
					mPublicTextView = (TextView) mWorkSpace.getChildAt(
							mWorkSpace.getCurrentScreen()).findViewById(
							0x7f0d0117);
				if (isUpdating)
	...
	private void initMain() {
		...
		mCityWeatherBgSwitcher = (CityWeatherBgSwitcher) getParent()
				.findViewById(0x7f0d01ad);
	...
	public void onClick(View view) {
		if (view == mCityManager || view == mCityNameView)
			doIntentCityManager();
		else if (view == mBroadcast)
			doBroadcast();
		else if (view == mUpdateWeather)
			doUpdateCityWeather(false);
		else if (view != mAdUmeng)
			;
	}
	...
	protected void onDestroy() {
		...
		super.onDestroy();
		if (mWeathearTabCallBackImpl != null)
			WeatherTabPublisher.getInstance().unSubscribe(
					mWeathearTabCallBackImpl);
	...
	protected void onResume() {
		...
		if (android.os.Build.VERSION.SDK_INT > 4)
			if (mIsFromCityView) {
				...
				if (mCurrentCityIdx != Gl.getCurrentCityIndex()) {
					mCurrentCityIdx = Gl.getCurrentCityIndex();
					((ViewGroup) findViewById(0x7f0d01d7))
							.startAnimation(mFadeAnim);
				}
			} else {
				((ViewGroup) findViewById(0x7f0d01d7))
						.startAnimation(mFadeAnim);
		...
		StatsUtil
				.updateStatsPv(com.moji.mjweather.util.StatsUtil.StatsPv.PV_WW);
	}
